...
def func2(params, v_shift, s,  unc, hitran_data, stel, o2, continuum,iline = None, mode='sum'):
    """
...
def define_pararr2(data,nlines,nspec,pstart=None):
	# For each, define name and value
	...
	par_lo = np.concatenate(( 
							...
							np.log10(data['S'])-0.15        # line strength
							))         
	...
	par_hi = np.concatenate((
                            ...
                            data['Shift0']+0.01,           # Shift0
                            np.zeros(nlines)+0.025,           # Shift2
							np.log10(data['S'])+0.1     # line strength
							))         
...
def func(params, v, s,  unc, hitran_data, stel_mod, o2_mod, iline = None, mode='sum'):
    ...
    shift0 = params[m + 3*nspec + nlines: m + 3*nspec + 2*nlines]
    strengths  = params[m + 3*nspec + 2*nlines: m + 3*nspec + 3*nlines]
...
#    ispec           = 0 
	...
	# Analyze outputs
    params = out['x']
    model,stel,o2,continuum,v_shift = func(params, v, s,  unc, data, stel_mod, o2_mod,mode='get_model')
    ...
    data['gamma-air']  = sigs0   # fill in whichever hitran variable deciding to vary
    data['Shift0'] = shift0
    data['S'] = 10**strengths
...
############### Part 2 of fit #############
    ...
    # Feed stel, o2, and params into func2
    pnames2,params_start2,bounds2 = define_pararr2(data,len(data['S']),len(s))                             

    out2 = opt.minimize(func2,params_start2,\
        args=(v_shift, s, unc, data, stel,o2,continuum),\
        method="SLSQP",bounds=bounds2,options={'maxiter' : 100}) 

	# Analyze outputs
    params2 = out2['x']
    model2 = func2(params2, v_shift, s,  unc, data, stel, o2,continuum,mode='get_model')
...
############################################################
    ...
    a0.plot(v,model2[ispec],'k--')
    a0.set_xlim([v[-3],v[2]])
    a0.set_ylabel('Absorbance')
    a0.set_title('Spectrum with Best Fit')

    a1.plot(v,s[ispec]-model2[ispec],'k--')
    a1.plot(v,s[ispec]-model[ispec],'g-')
    ...
    # Plot
    plt.figure(-11)
    p_lo = np.array(bounds2)[:,0]
    p_hi = np.array(bounds2)[:,1]
    frac =  (params2-p_lo)/(p_hi-p_lo)
    plt.plot(np.arange(len(params2)), frac,'o')
...
#    plt.scatter(np.arange(len(params))[getsigs], frac[getsigs], c=(62+np.log(data['S'])))
    ...
    #label regions
    for i,pname in enumerate(np.unique(pnames2)):
        indices = np.where(pnames2==pname)[0]
